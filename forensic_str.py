  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body {
      ...
      color: var(--text);
      background:
        linear-gradient(135deg, rgba(244, 192, 175, 0.88), rgba(247, 230, 211, 0.98) 58%, rgba(31, 70, 182, 0.12)),
        var(--cream);
      padding: 24px;
    }
    ...
    header {
      ...
      box-shadow: 0 14px 34px rgba(23, 50, 125, 0.10);
    ...
    .panel {
      ...
      box-shadow: 0 12px 30px rgba(23, 50, 125, 0.10);
      backdrop-filter: blur(16px);
      padding: 16px;
    }
    ...
    input[type="file"]::file-selector-button {
      border: 1px solid rgba(23, 50, 125, 0.16);
      border-radius: 6px;
    ...
    .chart-empty {
      ...
      min-height: 300px;
      place-items: center;
      border: 1px dashed rgba(247, 230, 211, 0.26);
      border-radius: 8px;
      background: rgba(255, 255, 255, 0.04);
      color: rgba(247, 230, 211, 0.86);
      text-align: center;
      line-height: 1.45;
      padding: 22px;
      font-size: 14px;
...
<body>
  <main>
    <header>
      <div>
        ...
        <p class="subtitle">Upload file DNA TKP dan database tersangka. Sistem membaca marker STR dari CSV, menghitung repeat terpanjang dengan RegEx, lalu membandingkan profil semua tersangka.</p>
      </div>
    ...
    <section class="layout">
      ...
      <article class="panel">
        <div class="panel-heading">
          ...
          <span class="panel-kicker">Comparison chart</span>
        </div>
        <div class="chart-frame">
          <div id="profileComparisonChart">
            <div class="chart-empty">Upload kedua file untuk melihat chart profil STR.</div>
          </div>
        </div>
      </article>
    </section>
  ...
  <script>
    ...
    function renderGroupedProfileChart(state) {
      ...
      const width = Math.max(760, markers.length * 138 + 160);
      const height = 340;
      const left = 58;
      const right = 24;
      const top = 50;
      ...
      svg += '<text x="18" y="29" fill="#F7E6D3" font-size="15" font-weight="800" font-family="Inter, Arial">Profil STR DNA TKP vs Tersangka</text>';
      svg += '<text x="18" y="45" fill="rgba(247,230,211,0.72)" font-size="10" font-family="Inter, Arial">Repeat count per marker</text>';
      ...
      markers.forEach((marker, markerIndex) => {
        ...
        svg += `<text x="${center}" y="${height - 38}" fill="#F7E6D3" font-size="12" font-weight="800" text-anchor="middle" font-family="Inter, Arial">${escapeHtml(marker)}</text>`;
