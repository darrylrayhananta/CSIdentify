...
Program ini mensimulasikan investigasi DNA forensik berbasis Short Tandem Repeats (STR). DNA dari TKP diprofilkan dengan RegEx untuk mencari jumlah pengulangan berurutan terpanjang pada setiap marker, lalu profil tersebut dibandingkan dengan database tersangka.
...
Setelah input diberikan, program menyimpan DNA ke CSV kasus, defaultnya `data/cases.csv`, lalu analisis membaca ulang DNA dari CSV tersebut. Program juga akan menampilkan jejak processing: pola RegEx yang dipakai, posisi run STR yang ditemukan, repeat terpanjang per marker, lalu skoring tiap tersangka.
...
- `output/investigation_dashboard.html`: aplikasi HTML interaktif untuk upload file `.txt` DNA TKP, upload file `.csv` tersangka, analisis STR, trace RegEx, chart profil STR DNA TKP vs tersangka, dan ranking tersangka.
...
STR adalah motif DNA pendek yang berulang secara tandem. Dalam model edukatif ini, profil seseorang direpresentasikan sebagai vektor jumlah repeat maksimum untuk beberapa marker, misalnya `AGAT=7` dan `AATG=4`. Perbandingan dilakukan pada vektor profil tersebut sehingga program tidak perlu melakukan alignment seluruh panjang DNA.
